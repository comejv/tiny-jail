...
use tiny_jail::fuzz;
use tiny_jail::io::SimilarityConfig;
...
struct ReduceArgs {
    ...
    #[arg(short = 'c', long, default_value_t = 3, value_name = "SIZE")]
    ...
    /// Pass the initial partitioning to the greedy phase.
    ///
    /// This will increase the number of tests performed.
    #[arg(short = 'g', long = "greedy-pass")]
    greedy_pass: bool,

    /// Load detailed syscall usage statistics from a file.
    ///
    /// This file can be generated by calling `tiny-jail exec --watch-all-logs --stats-output STATS_FILE -- my-program`.
    #[arg(short = 's', long, value_name = "FILE")]
    stats_file: Option<String>,

    /// Similarity presets: strict, default, lenient, or very-lenient
    ...
    /// If the similarity score is above this threshold, the user will be prompted to confirm.
    #[arg(long)]
    user_confirm_threshold: Option<f32>,

    /// Allow lines to be in different order.
    #[arg(long)]
...
fn run() -> Result<(), AppError> {
    ...
    match cli.command {
        ...
        Commands::Reduce(reduce_args) => {
            ...
            let similarity_config = match reduce_args.similarity_mode.as_str() {
                "strict" => SimilarityConfig::strict(),
                "lenient" => SimilarityConfig::lenient(),
                "very-lenient" => SimilarityConfig::very_lenient(),
                _ => SimilarityConfig::default(),
            };

            let mut similarity_config = similarity_config;
            if let Some(threshold) = reduce_args.fuzzy_threshold {
                similarity_config.fuzzy_lines = true;
                similarity_config.fuzzy_threshold = threshold;
            }
            if reduce_args.order_insensitive {
                similarity_config.order_insensitive = true;
            }
            ...
            let options = tiny_jail::options::ReduceProfileOptions {
                ...
                greedy_pass: reduce_args.greedy_pass,
                stats_file: reduce_args.stats_file,
                similarity_config,
            };
